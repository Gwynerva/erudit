...
import { AsideType, clickTargets, forcedAside, switchVisible } from '@scripts/aside';
...
const canShowSwitch = computed(() => switchVisible.value && !forcedAside.value);
...
onMounted(() => {
    clickTargets[props.type].push(...[switchElement.value, asideElement.value]);
});
...
<template>
    <div ref="switchElement" @click="switchClick" :class="[$style.asideSwitch, typeClass, canShowSwitch ? $style.visible : '']">
        <Icon name="left-aside-open" />
    </div>

    <aside ref="asideElement" :class="[$style.aside, typeClass, asideForceVisible ? $style.forceVisible : '']">
        <slot></slot>
    </aside>
</template>
...
@mixin ___asidePositionLogic
{
    ...
    &.major
    {
        ...
        @include bp.above('aside2') { --_posVisible: var(--_posVisibleMax); }
    }
...
.aside
{
    ...
    @mixin asideModal
    {
        background: color-mix(in srgb, color-mix(in srgb, var(--bgAside), black 0%), transparent 22%);
        ...
        @include dark
        {
            ...
            box-shadow:
                0px 0px 15px 15px rgba(255, 255, 255, .025),
                0px 0px 8px 0px rgba(255, 255, 255, .03);
        }
    }
    ...
    &.major
    {
        ...
        box-shadow:
            -1px 0px 2px 0px var(--shadowColor),
            -1px 0px 2px 0px var(--shadowColor) inset;
...
@mixin ___switchPositionLogic
{
    ...
    &.visible { --_pos: var(--_posVisible); }

    &.major
    {
        left: var(--_pos);

        @include bp.above('aside1') { --_posHidden: var(--_posBehindMajorAside); --_posVisible: var(--_posHidden); }
    }
    ...
    &.minor
    {
        ...
        @include bp.above('aside2') { --_posVisible: var(--_posHidden); }
    }

    &.major, &.minor { @include bp.above('aside2') { --_posHidden: var(--_posBehindFullLayout); } }
}
...
.asideSwitch
{
    ...
    &:not(.visible) { @include invisible; }
    &.major { @include bp.above('aside1') { @include invisible; } }
    @include bp.above('aside2') { @include invisible; }
}
</style>