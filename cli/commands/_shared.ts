...
export function prepareBuildDir()
{
    ...
    const tsconfig = {
        include: [
            CONTEXT.packageDir + '/package/global.d.ts', // For globally accessible types like `defineEruditConfig` and etc...
            '../erudit.ts',
            '../contributors/**/*',
            '../content/**/*',
        ]
    ...
    writeFileSync(CONTEXT.buildDir + '/tsconfig.json', JSON.stringify(tsconfig, null, 4), 'utf-8');
    writeFileSync(autoNuxtDir + '/nuxt.config.ts', nuxtConfig, 'utf-8');

    writeFileSync(CONTEXT.buildDir + '/__AUTOGENERATED__', '');
}